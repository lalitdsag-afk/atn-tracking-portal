...
# --- SIDEBAR LINK TO SAR STATUS APP ---
st.sidebar.link_button("🌐 Go to SAR Status App", "https://sar-esd.streamlit.app/", use_container_width=True)
st.sidebar.markdown("---")
...
# Render Authenticated Private Dashboards
if st.session_state["authenticated"]:
    ...
    if user_role == "DG (Director General)":
        ...
        if all_active and isinstance(all_active, list):
            ...
            for row in all_active:
                ...
                status = f"🌐 With {row['external_destination']}" if row.get('date_sent_external') else ("👑 With GO" if row.get('date_sent_to_fa') and row.get('date_sent_to_go') else ("💼 With F&A Cell" if row.get('date_sent_to_fa') else f"⏳ With Wing ({row['assigned_wing']})"))
    ...
    elif user_role == "Group Officer (GO)":
...
# --- 4. GLOBAL DASHBOARD (MASTER BOARD FOR UN-AUTHENTICATED & NON-DG USERS) ---
if user_role != "DG (Director General)":
    st.markdown("---")
    st.header("📊 Live Active Pipeline (Master Board)")
    
    # Secondary Dropdown Explorers
