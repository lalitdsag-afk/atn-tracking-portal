...
def init_db():
    ...
    # 1. Create ATN Table if it doesn't exist
    cursor.execute('''
    ...
    # 3. Seed initial standard accounts if the user table is empty
    cursor.execute("SELECT COUNT(*) FROM users")
    if cursor.fetchone()[0] == 0:
        initial_users = [
            ('dg_admin', 'dg123', 'DG (Director General)'),
            ('fa_nodal', 'fa123', 'F&A Cell (Nodal)'),
            ('go_officer', 'go123', 'Group Officer (GO)'),
            ('inspection_wing', 'wing123', 'Inspection'),
            ('ea_wing', 'wing123', 'EA'),
            ('bangalore_wing', 'wing123', 'Bangalore'),
            ('mumbai_wing', 'wing123', 'Mumbai'),
            ('kolkata_wing', 'wing123', 'Kolkata'),
            ('chennai_wing', 'wing123', 'Chennai')
        ]
        cursor.executemany("INSERT INTO users (username, password, role) VALUES (?, ?, ?)", initial_users)
        conn.commit()

    # 4. SAFETY MIGRATION: Forcefully inject 'remarks' column if missing from an older DB layout
    try:
...
if not st.session_state["authenticated"]:
    ...
    if st.sidebar.button("🔑 Log In"):
        cursor.execute("SELECT username, password, role FROM users WHERE username=? AND password=?", (input_uid, input_pwd))
        account = cursor.fetchone()
        if account:
            st.session_state["authenticated"] = True
            st.session_state["user_info"] = {"username": account[0], "role": account[2]}
            st.sidebar.success("Access Granted!")
            st.rerun()
        else:
            st.sidebar.error("❌ Invalid User ID or Password.")
    st.stop()  # Halt execution if user is not authorized yet
else:
    ...
    # ⚙️ Option to Change Password Tool
    with st.sidebar.expander("⚙️ Account Settings (Change Password)"):
        ...
        if st.button("Update Password"):
            if new_pwd == confirm_pwd and new_pwd.strip() != "":
                cursor.execute("UPDATE users SET password=? WHERE username=?", (new_pwd.strip(), current_user))
                conn.commit()
                st.success("Password updated successfully!")
            else:
                st.error("Passwords mismatch or invalid value.")
