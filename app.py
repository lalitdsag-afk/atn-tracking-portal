...
def init_db():
    ...
    try:
        ...
        # This catches the error if the column already exists, letting the app pass cleanly
        pass
...
if user_role == "F&A Cell (Nodal)":
    ...
    # SECTION B: VERIFICATION / SCRUTINY WORKFLOW (WHEN ITEMS COME BACK FROM WINGS)
    st.markdown("---")
...
if user_role == "Group Officer (GO)":
    ...
    if not go_items:
        ...
        for item in go_items:
            with st.expander(f"🔵 Action Required: Report {item[1]} [{item[4]}] (Submitted by F&A)", expanded=True):
                ...
                with col_d2:
                    dest = st.selectbox("External Destination", ["DGA", "F&C", "HQ"], key=f"go_dest_{item[0]}")
    ...
    # Track items that are currently sitting with external bodies for closure
    st.markdown("---")
    ...
    if not ext_items:
        st.caption("No files currently deployed with external branches.")
    else:
        for item in ext_items:
            with st.container():
                col_c1, col_c2 = st.columns([4, 1])
                with col_c1:
                    st.warning(f"📌 Report No. {item[1]} [{item[5]}] is currently **With {item[3]}** (Sent on: {item[4]}) [Allocated to: {item[2]}]")
                with col_c2:
                    close_btn = st.button("🔒 Mark as Closed", key=f"close_{item[0]}")
                
                with st.expander("View history / Append closure remark"):
                    if item[6]:
                        st.text_area("History Summary", value=item[6], disabled=True, key=f"ext_hist_{item[0]}")
                    final_remark = st.text_input("Add Final Closure Note (Optional)", key=f"close_rem_{item[0]}")
                ...
                if close_btn:
                    ...
                    conn.commit()
                    st.success("File permanently archived.")
                    st.rerun()
            st.markdown("---")
